...
function checkZodiac() {
    const day = Number(prompt("Nhập ngày sinh của bạn:"));
    const month = Number(prompt("Nhập tháng sinh của bạn:"));
    if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
        alert("Ngày hoặc tháng không hợp lệ. Vui lòng nhập lại.");
        return;
    }
    const zodiacSign = getCHD(day, month);
